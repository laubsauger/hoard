...
export const crowdFields = {
  ...
  crowdLimbedBudget: num({
    ...
    max: 512,
    integer: true,
    tiers: { 'desktop-high': 220, 'desktop-medium': 96, 'desktop-compat': 48, 'mobile-webgpu': 16 },
  }),
  crowdLimbedMaxSimTier: num({
    owner: 'rendering',
    unit: 'count',
    doc: 'SoA simTier <= this is rendered as a limbed figure (0 hero, 1 active, 2 abstract). Set to 2 so a DENSE near horde (where most nearby zombies are abstract-tier, not live) still reads as figures instead of a ring of boxes right next to the player — only the far impostor band stays boxes. The limbedBudget caps the figure count; distance/slot order decides which eligible slots win it.',
    default: 2,
    min: 0,
    max: 3,
    integer: true,
