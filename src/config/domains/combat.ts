...
export const combatConfig = registerDomain('combat', {
  ...
  hordeWanderRefreshTicks: num({
    owner: 'combat',
    unit: 'count',
    doc: 'Ticks an idle zombie holds one wander direction before re-rolling it (T137). Per-slot phase-offset so the crowd does not all turn at once. Kept short (~1.5 s at 30 Hz) so a body actually ROAMS — long intervals let a lone zombie amble out and straight back, reading as stuck near its spawn.',
    default: 45,
    min: 1,
    max: 100000,
    integer: true,
