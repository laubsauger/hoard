...
export const lightingConfig = registerDomain('lighting', {
  ...
  flashlightIntensity: num({
    ...
    min: 0,
    max: 200,
    tiers: { 'desktop-high': 20, 'mobile-webgpu': 12 },
  }),
  flashlightDayIntensityScale: num({
    owner: 'lighting',
    unit: 'ratio',
    doc: 'Fraction of flashlightIntensity that remains in full daylight (subtle by day, dominant at night).',
    default: 0.12,
    min: 0,
    max: 1,
  }),
  ...
  flashlightAngleMarginDegrees: num({
    owner: 'lighting',
    unit: 'degrees',
    doc: 'Extra half-angle added to the player FOV half-angle for the SpotLight cone so its penumbra covers the revealed wedge edge.',
    default: 8,
    min: 0,
    max: 60,
  }),
