...
export const lightingConfig = registerDomain('lighting', {
  ...
  sunIntensity: num({
    owner: 'lighting',
    unit: 'ratio',
    doc: 'Directional key-light intensity at midday (relative luminance multiplier). Raised 1→1.8 so a CLEAR noon reads BRIGHT + crisp (the clear weather grade keeps keyScale=1, so this IS the clear-day key); overcast/fog/smoke grades scale it down for their flatter vibe.',
    default: 1.8,
    min: 0,
    max: 10,
  }),
  ...
  moonColor: num({
    owner: 'lighting',
    unit: 'ratio',
    doc: 'Night key (moon) light tint as a packed 0xRRGGBB hex (cool blue moonlight). The per-weather grade tints the DAY key; the moon keeps this cool tone at night across all profiles.',
    default: 0xaebed8,
    min: 0,
    max: 0xffffff,
    integer: true,
  ...
  ambientIntensity: num({
    owner: 'lighting',
    unit: 'ratio',
    doc: 'Baked/precomputed indirect ambient floor for static architecture. Raised 0.25→0.33 so a clear day fill keeps shadowed faces readable + colourful without washing out; per-weather grades scale this (rain/fog lift it for soft flat light, smoke drops it).',
    default: 0.33,
    min: 0,
    max: 5,
  }),
  ...
  fogExtinctionPerMeter: num({
    owner: 'lighting',
    unit: 'ratio',
    doc: 'Base atmospheric extinction coefficient per world meter (clear weather). Lowered 0.006→0.005 so a CLEAR day stays crisp far into the distance (fog pushed out) — heavy weather still thickens via weatherExtinctionMultiplierMax.',
    default: 0.005,
    min: 0,
    max: 1,
  }),
  weatherExtinctionMultiplierMax: num({
    owner: 'lighting',
    unit: 'ratio',
    doc: 'Upper multiplier on fog extinction at maximum weather severity (rain/smoke/fog). Raised 6→8 to keep heavy weather thick after the lower base extinction (so fog/smoke still close the visibility right down).',
    default: 8,
    min: 1,
    max: 50,
  }),
  ...
  fogFarMaxMeters: num({
    owner: 'lighting',
    unit: 'meters',
    doc: 'Upper clamp on the fog far distance (clear weather) so distant geometry still fades into atmosphere (B5). Raised 360→480 so a clear day reads crisp to the horizon (fog barely-there) rather than a grey wash over the mid-distance.',
    default: 480,
    min: 10,
    max: 4000,
  }),
