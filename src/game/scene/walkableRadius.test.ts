...
describe('radius-aware edge-wall collision', () => {
  ...
  it('circleClearsEdges: body centred away from the wall clears, body poking across it does not', () => {
    const g = grid();
    expect(circleClearsEdges(g, center(2), center(1), 0.35)).toBe(true); // 2.5±0.35 stays in cell (2,1)
    expect(circleClearsEdges(g, 2.8, center(1), 0.35)).toBe(false); // 2.8+0.35=3.15 → into walled cell (3,1)
  });

  it('gridWalkableRadius rejects a body poking across a walled edge though both cells are walkable', () => {
    const g = grid();
    expect(gridWalkableWorld(g, 2.8, center(1))).toBe(true); // centre cell is walkable
    expect(gridWalkableWorld(g, 3.15, center(1))).toBe(true); // neighbour cell is walkable too
    expect(gridWalkableRadius(g, 2.8, center(1), 0.35)).toBe(false); // …but the body overlaps the wall
  ...
  it('isWalkableRadius uses the scene navGrid for edge-walls; a mock without one stays cell-only', () => {
    ...
    expect(isWalkableRadius(scene, 2.8, center(1), 0.35)).toBe(false); // pokes across the wall → rejected
    ...
    expect(isWalkableRadius(cellOnly, 2.8, center(1), 0.35)).toBe(true);
  });
});
