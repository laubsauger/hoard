...
| id | st | lane | task | deps | cites |
|---|---|---|---|---|---|
| T87 | ~ | S/R | LARGE multi-building suburban district: street grid (asphalt+sidewalk+grass) carrying ≥8 separately-enterable human-scale houses (perimeter walls, front door, interior rooms, windows, fenced yard, 1–2 storeys), cars/tires/overgrowth dressing, a central green where the horde musters + streams outward; ONE house keeps the destructible dividing wall. [DONE: `TestBlock` contract (`buildings[]`/`groundRects[]`/`props[]`, `buildingsOf`) + `buildCityDistrict` multi-building layout merged. TODO: blockScene multi-building RENDER + per-building cutaway (main's single-building blockScene kept on merge; additive render patch pending)] | T40,T43,T70,B22 | V59,V20,V37,world-config,ART-DIRECTION |
