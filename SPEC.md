...
### Wave 10 — World scale (the LARGE explorable district)

Reframes the authored world from "one defended building" (the M1/M2 slices were scoping artifacts) to a Project-Zomboid-scale multi-building district the player explores. Extends the `TestBlock` scene contract (additive, back-compat) + the renderer; the horde sim / streaming / persistence / objective ride it unchanged.

| id | st | lane | task | deps | cites |
|---|---|---|---|---|---|
| T80 | x | S/R | LARGE multi-building suburban district: street grid (asphalt+sidewalk+grass) carrying ≥8 separately-enterable human-scale houses (perimeter walls, front door, interior rooms, windows, fenced yard, 1–2 storeys), cars/tires/overgrowth dressing, a central green where the horde musters + streams outward; ONE house keeps the destructible dividing wall. Extend `TestBlock` (`buildings[]`/`groundRects[]`/`props[]`, `buildingsOf`), rewrite `buildCityDistrict`, multi-building walls/floor/roof + PER-BUILDING cutaway in `blockScene`; streaming sectors on open streets | T40,T43,T70,B22 | V57,V20,V37,world-config,ART-DIRECTION |
